...
- **Fonts**: Figtree (body via `font-sans`), Advercase (headings via `font-heading`, local font in `app/fonts/`), DM Mono (mono via `font-mono`) -- loaded in `app/layout.tsx` as CSS variables
